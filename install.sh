...
function check_ports() {
    ...
    TCP_PLAN="3333	nova-objectstore
5000	keystone-all
6080	novnc
8080	focus
8773	nova-api
...
123	ntpd"
    ...
    UDP_MASK="${UDP_MASK%|}"
    ALTAI_BUSY_PORTS=$(echo "$NETSTAT" | grep -E "^(tcp.*:($TCP_MASK)|udp.*:($UDP_MASK))")

    if [ -n "$ALTAI_BUSY_PORTS" ]; then
        error_start
        echo "Some ports are that are used by Altai services are already listened."
...
function get_param() {
    receipt="$receipt" python -c 'import json; import os; print json.load(open(os.environ["receipt"]))["'$1'"]' || die "cannot read parameter $1"
}

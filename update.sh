...
function determine_versions() {
    ...
    altai_node_json="$altai_installer_dir/altai-node.json"
    if [ ! -r "$altai_node_json" ]; then
        mv "$altai_installer_dir/$(has_role master && echo master-node.json || echo compute-node.json)" "$altai_node_json"
    fi
}
